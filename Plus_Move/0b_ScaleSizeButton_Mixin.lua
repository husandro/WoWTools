...
local function Set_Init_Frame(btn, target, size, initFunc)
    if WoWTools_FrameMixin:IsLocked(target) then--not InCombatLockdown() or not sel:IsProtected() 
        ...
        if WoWTools_DataMixin.Player.husandro then
            print(WoWTools_MoveMixin.addName, '|cff9e9e9e'..issecure()..'|r', target:GetName(), '|cnWARNING_FONT_COLOR:不能执行')
        end
    else
        if size then
        ...
        if WoWTools_DataMixin.Player.husandro then
            print(WoWTools_MoveMixin.addName, '|cff9e9e9e'..issecure()..'|r', target:GetName(), '|cnGREEN_FONT_COLOR:执行')
        end
    end
end
