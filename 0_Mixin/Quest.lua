...
function WoWTools_QuestMixin:GetQuestAll()
    ...
    GameTooltip:AddDoubleLine((e.onlyChinese and '已完成' or  CRITERIA_COMPLETED)..' '..WoWTools_Mixin:MK(#all, 3), WoWTools_QuestMixin:GetColor('Day').hex..(e.onlyChinese and '日常' or DAILY)..': '..GetDailyQuestsCompleted()..format('|A:%s:0:0|a', e.Icon.select))
    GameTooltip:AddLine(e.Player.col..(e.onlyChinese and '上限' or CAPPED)..': '..(numQuest+ dayNum+ weekNum)..'/'..(C_QuestLog.GetMaxNumQuestsCanAccept() or 38))
    GameTooltip:AddLine(' ')
    GameTooltip:AddLine('|cnGREEN_FONT_COLOR:'..(e.onlyChinese and '当前地图' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, REFORGE_CURRENT, WORLD_MAP))..': '..inMapNum)
    GameTooltip:AddLine(' ')
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Day').hex..(e.onlyChinese and '日常' or DAILY)..': '..dayNum)
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Week').hex..(e.onlyChinese and '周长' or WEEKLY)..': '..weekNum)
    GameTooltip:AddLine((num>=MAX_QUESTS and '|cnRED_FONT_COLOR:' or '|cffffffff')..(e.onlyChinese and '一般' or RESISTANCE_FAIR)..': '..numQuest..'/'..MAX_QUESTS)
    GameTooltip:AddLine(' ')
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Legendary').hex..(e.onlyChinese and '传说' or GARRISON_FOLLOWER_QUALITY6_DESC)..': '..legendaryNum)
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Legendary').hex..(e.onlyChinese and '战役' or TRACKER_HEADER_CAMPAIGN_QUESTS)..': '..campaignNum)
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Legendary').hex..(e.onlyChinese and '悬赏' or PVP_BOUNTY_REWARD_TITLE)..': '..bountyNum)
    GameTooltip:AddLine(WoWTools_QuestMixin:GetColor('Legendary').hex..(e.onlyChinese and '故事' or 'Story')..': '..storyNum)
    GameTooltip:AddLine((e.onlyChinese and '追踪' or TRACK_QUEST_ABBREV)..': '..C_QuestLog.GetNumQuestWatches())
end
