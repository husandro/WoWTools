...
local function Init()
    ProfessionMicroButton:HookScript('OnEnter', function()
        ...
        if prof1 and prof1>0 then
            ...
            if name then
                prof1Text='|T'..(icon or 0)..':0|t'..(bat and '|cnRED_FONT_COLOR:' or '|cffffffff')..name..'|r'..e.Icon.mid..(WoWTools_Mixin.onlyChinese and '上' or HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_UP)
            end
        end
        ...
        if prof2 and prof2>0 then
            ...
            if name then
                prof2Text='|T'..(icon or 0)..':0|t'..(bat and '|cnRED_FONT_COLOR:' or '|cffffffff')..name..'|r'..e.Icon.mid..(WoWTools_Mixin.onlyChinese and '下' or HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_DOWN)
            end
        end
