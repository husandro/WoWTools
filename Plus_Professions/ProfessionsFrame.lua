...
local function Init()
    ...
    hooksecurefunc(ProfessionsRecipeListRecipeMixin, 'OnEnter', function(self)
        ...
        if info.itemLevel or info.skillLineAbilityID then
            GameTooltip:AddDoubleLine(info.skillLineAbilityID and 'skillLineAbilityID '..info.skillLineAbilityID,  info.itemLevel and info.itemLevel>1 and format(e.onlyChinese and '物品等级%d' or ITEM_LEVEL, info.itemLevel))
        end
    ...
    hooksecurefunc(ProfessionsFrame.CraftingPage.SchematicForm, 'Init', function(self, recipeInfo)--, isRecraftOverride)
        ...
        if not btn then
            ...
            btn:SetScript('OnEnter', function(self2)
                ...
                GameTooltip:AddLine(' ')
                GameTooltip:AddDoubleLine(e.onlyChinese and '自动加入' or AUTO_JOIN, e.GetEnabeleDisable(not Save().disabledEnchant))
    ...
    hooksecurefunc(ProfessionsFrame.SpecPage, 'UpdateDetailedPanel', function(self, setLocked)
        ...
        if not button then
            ...
            button:SetScript('OnEnter', function(self2)
                ...
                GameTooltip:ClearLines()
                GameTooltip:AddDoubleLine(not e.onlyChinese and PROFESSIONS_SPECS_ADD_KNOWLEDGE or "运用知识", e.onlyChinese and '全部' or ALL)
                GameTooltip:AddDoubleLine(WoWTools_Mixin.addName, WoWTools_ProfessionMixin.addName)
                GameTooltip:Show()
            end)
