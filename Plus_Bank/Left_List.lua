...
local function Set_Tooltip(self, tooltip, type)
    ...
    if type==0 or type==1 then
        for itemID, count in pairs(self.bankItems or {}) do
            if find==0 then
                tooltip:AddDoubleLine(' ', (WoWTools_DataMixin.onlyChinese and '银行' or BANK)..'|A:Banker:0:0|a')
            end
    ...
    if type==0 or type==2 then
        for itemID, count in pairs(self.bagItems or {}) do
            if find2==0 then
                tooltip:AddDoubleLine(' ', (WoWTools_DataMixin.onlyChinese and '背包' or INVTYPE_BAG)..'|A:bag-main:0:0|a')
            end
            find2=find2+1
            name= WoWTools_ItemMixin:GetName(itemID, nil, nil, {notCount=true})
            col= select(4, WoWTools_ItemMixin:GetColor(nil, {itemID=itemID}))
...
--提取             
    ...
    sub:SetTooltip(function(tooltip)
        ...
        if find==0 and find2==0 then
            tooltip:AddLine(tooltip:AddLine('|A:common-icon-rotateright:0:0|a'..(WoWTools_DataMixin.onlyChinese and '银行' or BANK)))
        end
    end)
...
--存放
    ...
    sub:SetTooltip(function(tooltip)
        ...
        if find==0 and find2==0 then
            tooltip:AddLine('|A:common-icon-rotateright:0:0|a'..(WoWTools_DataMixin.onlyChinese and '背包' or HUD_EDIT_MODE_BAGS_LABEL))
        end
    end)
...
--数量
    ...
    function btn:set_tooltip()
        ...
        if find==0 and find2==0 then
            GameTooltip:AddLine((WoWTools_DataMixin.onlyChinese and '提取/存放' or (WITHDRAW..'/'..DEPOSIT))..WoWTools_DataMixin.Icon.left)
...
--提示，当前银行，类型
    function ListButton:set_bank_type()
        ...
        local text=
            index==1 and (WoWTools_DataMixin.onlyChinese and '银行' or BANK)
            or index==2 and (WoWTools_DataMixin.onlyChinese and '材料' or BANK_TAB_ASSIGN_REAGENTS_CHECKBOX)
            or index==3 and (WoWTools_DataMixin.onlyChinese and '战团' or ACCOUNT_QUEST_LABEL)
--空栏位
    ...
    function ListButton:set_tooltip()
        ...
        GameTooltip:AddDoubleLine(WoWTools_DataMixin.addName, WoWTools_BankMixin.addName)
        GameTooltip:AddLine(' ')
