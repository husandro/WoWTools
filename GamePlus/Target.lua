...
panel:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        if arg1==id then
            ...
            sel2.text:SetText(e.onlyChinse and '怪物数量' or CREATURE..AUCTION_HOUSE_QUANTITY_LABE)
            ...
            sel2:SetScript('OnEnter', function(self2)
                ...
                e.tips:AddDoubleLine('|cffffffff'..(e.onlyChinse and '怪物' or PLAYERS_IN_GROUP), e.onlyChinse and '数量' or AUCTION_HOUSE_QUANTITY_LABE)
                e.tips:AddLine(' ')
                e.tips:AddDoubleLine(e.onlyChinse and '显示敌方姓名板' or BINDING_NAME_NAMEPLATES, e.GetEnabeleDisable(C_CVar.GetCVarBool("nameplateShowEnemies")))
                e.tips:Show()
            end)
