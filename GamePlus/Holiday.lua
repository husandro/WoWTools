        ...
        if namesReady then
            for index, button in pairs(CalendarCreateEventInviteList.ScrollBox:GetFrames()) do--ScrollBox.lua
                ...
                if inviteInfo and inviteInfo.guid then
                   button.Class:SetText(e.GetPlayerInfo({unit=nil, guid=inviteInfo.guid, name=inviteInfo.name, reFriendFaction=true, reName=false, reRealm=false, reLink=false}))
                end
            end
        end
    end)
        ...
        securecall('UIDropDownMenu_Initialize', menu, function(self, level, type)
            ...
            for i=1 ,BNGetNumFriends() do
                ...
                if wowInfo and wowInfo.playerGuid and wowInfo.characterName and not inviteTab[wowInfo.characterName] then
                    ...
                    local text= e.GetPlayerInfo({unit=nil, guid=wowInfo.playerGuid, name=wowInfo.characterName, reFriendFaction=true, reName=true, reRealm=true, reLink=false})--角色信息
        ...
        securecall('UIDropDownMenu_Initialize', last, function(self, level, type)
            ...
            for index=1,  GetNumGuildMembers() do
                ...
                if name and guid and not inviteTab[name] and isOnline and name~=e.Player.name_server then
                    local text=e.GetPlayerInfo({unit=nil, guid=guid, name=name, reFriendFaction=true, reName=true, reRealm=true, reLink=false})--名称
