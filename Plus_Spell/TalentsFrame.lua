...
local function Init_Texture_Menu(self, root)
    ...
    for name in pairs(Save().bg.texture) do
        local isAtlas, textureID, icon= WoWTools_TextureMixin:IsAtlas(name, 240)
        if textureID then
...
--显示背景
    ...
    sub2:SetTooltip(function (tooltip)
        local _, textureID, icon= WoWTools_TextureMixin:IsAtlas(Save().bg.icon, 240)
        if textureID then
    ...
    function()
        ...
        nil,
        {
            ...
            OnShow=function(s)
                ...
                s.editBox:SetText('Interface\\AddOns\\WoWTools\\Sesource\\Bg\\')
            end,
            SetValue= function(s)
                local textureID,a= select(2, WoWTools_TextureMixin:IsAtlas(s.editBox:GetText(), 0))
                if textureID then
                    print(textureID, a )
                    Save().bg.icon= textureID
                    print('|T'..textureID..':120:120|t')
                    Call_Bg()
