        ...
        if btn and index<= numItem then
            ...
            if num and not btn.buyItemNum then
                ...
                btn.buyItemNum:SetScript('OnEnter', function(self)
                    ...
                    GameTooltip:AddLine(' ')
                    GameTooltip:AddDoubleLine('|T236994:0|t'..(e.onlyChinese and '自动购买物品' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, SELF_CAST_AUTO, PURCHASE)), not Save().notAutoBuy and Save().buyItems[self.itemID] or (e.onlyChinese and '无' or NONE))
                    local all= C_Item.GetItemCount(self.itemID, true, false, true)
                    local bag2= C_Item.GetItemCount(self.itemID)
                    GameTooltip:AddDoubleLine('|A:Banker:0:0|a'..(e.onlyChinese and '数量' or AUCTION_HOUSE_QUANTITY_LABEL), all..'= '.. '|A:bag-main:0:0|a'.. bag2..'+ '..'|A:Banker:0:0|a'..(all-bag))
					GameTooltip:Show()
                end)
            end
            --物品，属性
