...
function WoWTools_InviteMixin:Inv_All_Unit()--邀请，周围玩家
    ...
    if not self:Get_Leader() then--取得权限
        print(WoWTools_Mixin.addName, self.addName, '|cnRED_FONT_COLOR:', WoWTools_Mixin.onlyChinese and '你没有权利这样做' or ERR_GUILD_PERMISSIONS)
        return

    elseif UnitAffectingCombat('player') and (not p or not all) then
        print(WoWTools_Mixin.addName, self.addName, '|cnRED_FONT_COLOR:'..(WoWTools_Mixin.onlyChinese and '战斗中' or COMBAT))
        return
    end
    ...
    InvPlateTimer=C_Timer.NewTimer(0.3, function()
                    ...
                    if name and name~=UNKNOWNOBJECT and guid and not UnitInAnyGroup(u) and not UnitIsAFK(u) and UnitIsConnected(u) and UnitIsPlayer(u) and UnitIsFriend(u, 'player') and not UnitIsUnit('player',u) then
                        if not WoWTools_InviteMixin.InvPlateGuid[guid] then
                            ...
                            if not raid and n +co>=5  then
                                print(WoWTools_Mixin.addName, self.addName, format(PETITION_TITLE, '|cff00ff00'..(WoWTools_Mixin.onlyChinese and '转团' or CONVERT_TO_RAID)..'|r'))
                                break
                            end
                            n=n+1
                        end
        ...
        if n==1 then
            print(WoWTools_Mixin.addName, self.addName, WoWTools_Mixin.onlyChinese and '邀请成员' or GUILDCONTROL_OPTION7, '|cnRED_FONT_COLOR:'..(WoWTools_Mixin.onlyChinese and '无' or NONE))
        end
    end)
end
