            ...
            if Save().Channel and text and text:find(Save().ChannelText) then
                ...
                if co<5 or (IsInRaid() and co<40) then
                    ...
                    if guid and name and name~=WoWTools_DataMixin.Player.ame_server then
                        ...
                        print(WoWTools_DataMixin.Icon.icon2..WoWTools_InviteMixin.addName, WoWTools_DataMixin.onlyChinese and '频道' or CHANNEL, WoWTools_UnitMixin:GetLink(name, guid))
                    end
                end
            end
        end
