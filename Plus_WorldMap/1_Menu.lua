...
local function ShowHideTitle()
    ...
    if not Save().HideTitle then
        if WorldMapFrame:IsMaximized() then--WorldMapMixin:SetupTitle()
            text= e.onlyChinese and '地图' or WORLD_MAP
        else
            text= e.onlyChinese and '地图和任务日志' or MAP_AND_QUEST_LOG
        end
    end
...
local function Init_PlayerXY_Option_Menu(self, root2)
    ...
    sub:SetTooltip(function(tooltip)
        tooltip:AddLine(e.onlyChinese and '分享链接至聊天栏' or CLUB_FINDER_LINK_POST_IN_CHAT)

        local mapID= C_Map.GetBestMapForUnit("player")
        local can= mapID and C_Map.CanSetUserWaypointOnMap(mapID)
        if not can then
            tooltip:AddLine('|cnRED_FONT_COLOR:'..(e.onlyChinese and '当前地图不能标记' or "Cannot set waypoints on this map"))
        end
    end)
    ...
    if self==_G['WoWTools_PlayerXY_Button'] then
        root= root:CreateButton(
            ...
            ..(e.onlyChinese and '选项' or GAMEMENU_OPTIONS),
        function()
            return MenuResponse.Open
        end)
    end
    ...
    root:CreateCheckbox(
        e.onlyChinese and '右边' or HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_RIGHT,
    function()
...
--地图和任务日志
    sub= root:CreateCheckbox(
        e.onlyChinese and '地图和任务日志' or MAP_AND_QUEST_LOG,
    function()
...
--显示地图ID
    sub= root:CreateCheckbox(
        (e.onlyChinese and '地图' or WORLD_MAP)..' ID',
    function()
...
--地图坐标
    sub= root:CreateCheckbox(
        (e.onlyChinese and '地图' or WORLD_MAP)..' XY',
    function()
    ...
    WoWTools_MenuMixin:CreateSlider(sub, {
        ...
        name=e.onlyChinese and '宽度' or HUD_EDIT_MODE_SETTING_CHAT_FRAME_WIDTH,
        minValue=50,
        maxValue=300,
        step=1,
        bit=nil,
...
--AreaPOI名称
    ...
    sub:SetTooltip(function(tooltip)
        tooltip:AddLine(e.onlyChinese and '显示名称' or PROFESSIONS_FLYOUT_SHOW_NAME)
        tooltip:AddLine(e.onlyChinese and '需求刷新' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, NEED, REFRESH))
    end)

--地下城，加名称
    sub=root:CreateCheckbox(
        '|A:Dungeon:0:0|a'..(e.onlyChinese and '地下城' or DUNGEONS),
    function()
...
--世界地图任务，加名称
    sub=root:CreateCheckbox(
        '|A:Quest-Campaign-Available:0:0|a'..(e.onlyChinese and '世界任务' or WORLD_MAP_FILTER_LABEL_WORLD_QUESTS_SUBMENU),
    function()
