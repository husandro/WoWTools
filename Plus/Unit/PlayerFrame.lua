...
local function Craete_assisterButton()
    ...
    WoWTools_DataMixin:Hook('PlayerFrame_UpdatePartyLeader', function()
        ...
        contextual.assisterButton:SetShown((isLeader or WoWTools_DataMixin.Player.husandro) and IsInRaid())
...
--提示
    ...
    function frame.instanceFrame.dungeon:set_tooltip()
        ...
        for index, ID in pairs(tab) do
            ...
            local set
            if index==3 then
                set= ((UnitIsGroupLeader("player") or not IsInGroup()) and dungeonID~=ID and '|cnGREEN_FONT_COLOR:' or '|cff626262')
...
local function Create_keystoneFrame(frame)
    ...
    function frame.keystoneFrame:set_settings()
        ...
        self:SetShown(
            (WoWTools_DataMixin.Player.IsMaxLevel and not PlayerIsTimerunning())
