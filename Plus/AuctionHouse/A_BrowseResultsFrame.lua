...
--室内, 室外
                if entryInfo.isAllowedIndoors then
                    text= '|A:house-room-limit-icon:0:0|a'
                end
                if entryInfo.isAllowedOutdoors then
                    text= (text or '')..'|A:house-outdoor-budget-icon:0:0|a'
                end
--XP
...
--数量
                if btn.entryInfo.showQuantity then
                    stats= (entryInfo.numPlace or 0) + (entryInfo.quality or 0)+ (entryInfo.remainingRedeemable or 0)..'|A:house-chest-icon:0:0|a'
                end
            end

        elseif battlePetSpeciesID then
--宠物
...
--宝石插槽
            ...
            for n= 1, numSockets do
                text= (text or '')..'|A:socket-cogwheel-closed:0:0|a'
            end
        end
    end
...
local function Load_Item(btn, itemID)
...
local function Set_BrowseResultsFrame(frame)
    ...
    for _, btn in pairs(frame:GetFrames() or {}) do
        ...
        if itemKeyInfo and itemKeyInfo.itemID then
            Load_Item(btn, itemKeyInfo.itemID)
        end
    end
end
...
--购买，选定物品 AuctionHouseFrame.ItemBuyFrame.ItemDisplay.ItemButton
    ...
    hooksecurefunc( AuctionHouseItemListMixin, 'OnUpdate', function()
        print('aa')
    end)
