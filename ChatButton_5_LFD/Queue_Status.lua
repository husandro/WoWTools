...
local function get_InviteButton_Frame(index)
    ...
    if not frame then
        ...
        frame.InviteButton:SetScript('OnEnter', function(self2)
            ...
            GameTooltip:AddDoubleLine(self2:GetParent().applicantID, '|cnGREEN_FONT_COLOR:'..(e.onlyChinese and '邀请' or INVITE))
        ...
        frame.ChatButton:SetScript('OnEnter', function(self2)
            GameTooltip:SetOwner(self2, "ANCHOR_LEFT")
            GameTooltip:ClearLines()
            GameTooltip:AddDoubleLine( self2:GetParent().name, e.onlyChinese and '/密语' or SLASH_SMART_WHISPER2)
            GameTooltip:AddLine(self2:GetParent().tooltip)
            GameTooltip:Show()
        end)
        ...
        frame.DeclineButton:SetScript('OnEnter', function(self2)
            GameTooltip:SetOwner(self2, "ANCHOR_LEFT")
            GameTooltip:ClearLines()
            GameTooltip:AddDoubleLine( self2:GetParent().applicantID, '|cnRED_FONT_COLOR:'..(e.onlyChinese and '拒绝' or DECLINE))
            GameTooltip:AddLine(self2:GetParent().tooltip)
            GameTooltip:Show()
        end)
...
local function Init()
    ...
    function Button:set_tooltip()
        ...
        GameTooltip:ClearLines()
        GameTooltip:AddDoubleLine(WoWTools_LFDMixin.addName, e.onlyChinese and '列表信息' or  format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, SOCIAL_QUEUE_TOOLTIP_HEADER, INFO))
        GameTooltip:AddLine(' ')
