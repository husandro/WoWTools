...
local function Init()
    ...
    for index, text in pairs(EmojiText) do
        TextToTexture['{'..text..'}']= '|TInterface\\Addons\\WoWTools\\0_Sesource\\Emojis\\'..EmojiText_EN[index]..':0|t'
    end
    ...
    function EmojiButton:get_texture(index)
        index= index or Save.clickIndex or 18
        if index<=self.numFile then
            return 'Interface\\Addons\\WoWTools\\0_Sesource\\Emojis\\'..EmojiText_EN[index]
        else
...
panel:SetScript('OnEvent', function(self, event, arg1)
    if event=='ADDON_LOADED' then
        if arg1 == id then
            ...
            addName= '|TInterface\\Addons\\WoWTools\\0_Sesource\\Emojis\\Embarrass:0|tEmoji'
