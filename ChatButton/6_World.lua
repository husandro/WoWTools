...
local function Init_User_Chat_Filter()
    Menu.ModifyMenu("MENU_UNIT_FRIEND", function(_, root, data)
        ...
        sub:SetTooltip(function(tooltip, description)
            tooltip:AddDoubleLine(WoWTools_DataMixin.addName, addName)
...
--设置, 屏蔽刷屏, 数量
    sub2=sub:CreateButton('     '..(WoWTools_DataMixin.onlyChinese and '设置' or SETTINGS)..' |cnGREEN_FONT_COLOR:'..Save().myChatFilterNum, function()
        StaticPopupDialogs['WoWToolsChatButtonWorldMyChatFilterNum']= {
            ...
            OnAccept = function(self)
                ...
                print(WoWTools_DataMixin.addName, WoWTools_TextMixin:CN(addName), get_myChatFilter_Text())
            end,
...
--屏蔽玩家
    sub2=sub:CreateButton('     '..(WoWTools_DataMixin.onlyChinese and '屏蔽玩家' or IGNORE_PLAYER)..' #'..filterPlayer, function()
...
--全部清除
        sub2:CreateButton('|A:bags-button-autosort-up:0:0|a'..(WoWTools_DataMixin.onlyChinese and '全部清除' or CLEAR_ALL)..' #'..filterPlayer, function()
        ...
        for guid, num in pairs(Save().myChatFilterPlayers) do
            ...
            sub3=sub2:CreateButton('|cff9e9e9e'..index..')|r '..name..' |cff9e9e9e#'.. WoWTools_Mixin:MK(num, 3)..'|r', function(data)
                ...
                if Save().myChatFilterPlayers[data.guid] then
                    print(WoWTools_DataMixin.Icon.icon2.. addName, '|cnGREEN_FONT_COLOR:'..(WoWTools_DataMixin.onlyChinese and '移除' or REMOVE)..'|r', player)
                else
                    print(WoWTools_DataMixin.Icon.icon2.. addName, '|cff9e9e9e'..(WoWTools_DataMixin.onlyChinese and '尚未发现' or TAXI_PATH_UNREACHABLE)..'|r', player)
                end
...
--全部屏蔽, 现有列表
    sub:CreateButton('|A:GreenCross:0:0|a'..(WoWTools_DataMixin.onlyChinese and '全部添加' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, ALL, ADD))..' #'..filterNum, Set_Add_All_Player_Filter)


--全部清除, 屏蔽刷屏
    sub:CreateButton('|A:bags-button-autosort-up:0:0|a'..(WoWTools_DataMixin.onlyChinese and '全部清除' or CLEAR_ALL)..' #'..filterNum, function()
        FilterTextTab={}
    end)
        ...
        sub2:CreateButton(WoWTools_DataMixin.onlyChinese and '屏蔽' or IGNORE, function(data)
            for guid, name in pairs(data.guid or {}) do
                ...
                if Save().myChatFilterPlayers[guid] then
                    ...
                    Save().myChatFilterPlayers[guid]= 1
                    print(WoWTools_DataMixin.Icon.icon2.. addName, '|cnGREEN_FONT_COLOR:'..(WoWTools_DataMixin.onlyChinese and '屏蔽' or IGNORE)..'|r', player)
                end
            end
...
local function Init_User_Filter_Menu(root)
    ...
    sub= root:CreateCheckbox((WoWTools_DataMixin.onlyChinese and '自定义屏蔽' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, CUSTOM, IGNORE)).. ' '.. useNum, function()
        return Save().userChatFilter
    end, function()
        Save().userChatFilter= not Save().userChatFilter and true or false
        Set_Filter()
...
if not Save().userChatFilter then
    ...
    sub:CreateButton(WoWTools_DataMixin.onlyChinese and '添加' or ADD, function()
        StaticPopupDialogs['WoWTools_ChatButton_Wolrd_userChatFilterADD']= {
            ...
            OnAccept = function(self)
                ...
                print(WoWTools_DataMixin.Icon.icon2.. addName, WoWTools_DataMixin.onlyChinese and '添加' or ADD, text, WoWTools_UnitMixin:GetPlayerInfo(nil, nil, text, {reName=true, reRealm=true, reLink=true}))
            end,
            EditBoxOnTextChanged=function(self)
    ...
    --全部清除, 自定义屏蔽
        sub:CreateButton('|A:bags-button-autosort-up:0:0|a'..(WoWTools_DataMixin.onlyChinese and '全部清除' or CLEAR_ALL)..' #'..useNum, function()
        ...
        for name, tab in pairs(Save().userChatFilterTab) do
            ...
            sub2=sub:CreateButton(player..' '..tab.num, function(data)
                if Save().userChatFilterTab[data.name] then
                    print(WoWTools_DataMixin.Icon.icon2.. addName, WoWTools_DataMixin.onlyChinese and '移除' or REMOVE, WoWTools_UnitMixin:GetPlayerInfo({name=data.name, guid=data.tab.guid, reName=true, reRealm=true, reLink=true}))
...
local function Add_Menu(root, name, channelNumber)
    ...
    sub:SetTooltip(function(tooltip, desc)
        ...
        local t
        if value==0 then--不存在
            t= Get_Channel_Color(nil, 0)..(WoWTools_DataMixin.onlyChinese and '尚未发现' or TAXI_PATH_UNREACHABLE)
        elseif value==1 then
            t= WoWTools_DataMixin.onlyChinese and '已加入' or CLUB_FINDER_JOINED
        elseif value==2 then--屏蔽
            t= Get_Channel_Color(name, 2)..(WoWTools_DataMixin.onlyChinese and '已屏蔽' or IGNORED)
        end
...
panel:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        if arg1== 'WoWTools' then
            ...
            addName= '|A:tokens-WoW-generic-regular:0:0|a'..(WoWTools_DataMixin.onlyChinese and '频道' or CHANNEL)
