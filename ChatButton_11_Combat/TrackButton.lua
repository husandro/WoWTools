...
local function Init()--设置显示内容, 父框架TrackButton, 内容btn.text
    ...
    function TrackButton:set_tooltip()
        ...
        e.tips:AddLine(' ')
        e.tips:AddDoubleLine(WoWTools_Mixin.addName, WoWTools_CombatMixin.addName)
        e.tips:Show()
    end
...
function WoWTools_CombatMixin:Set_Combat_Tooltip(tooltip)
    ...
    tooltip:AddDoubleLine(
        (e.onlyChinese and '本周%s' or CURRENCY_THIS_WEEK):format('CD')..' ('..format(e.onlyChinese and '%d周' or WEEKS_ABBR, date('%W'))..')',
