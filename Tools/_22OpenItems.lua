    ...
    for bag=0, NUM_BAG_SLOTS do
        for slot=1, C_Container.GetContainerNumSlots(bag) do
            ...
            if info and info.itemID and info.hyperlink and not info.isLocked and info.iconFileID then
                ...
                elseif not Save.no[info.itemID] and not noItemTable[noItemTable] and not e.GetTooltipData(true, nil, nil, {bag=bag, slot=slot}) then--不出售, 可以使用
...
--                        if spell and IsUsableItem(info.hyperlink) and not C_Item.IsAnimaItemByID(info.hyperlink) then
                        if IsUsableItem(info.hyperlink) and IsConsumableItem(info.hyperlink) and not C_Item.IsAnimaItemByID(info.hyperlink) then
                            setAtt(bag, slot, info.iconFileID, info.itemID)
                            return
                        end
