...
local PointTab={
    {value=1, text=WoWTools_Mixin.onlyChinese and '左' or HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_LEFT},
    {value=2, text=WoWTools_Mixin.onlyChinese and '右' or HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_RIGHT},
    {value=3, text=WoWTools_Mixin.onlyChinese and '默认' or DEFAULT},
    '-',
    {value=4, text=WoWTools_Mixin.onlyChinese and '左|右' or (HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_LEFT..'|'..HUD_EDIT_MODE_SETTING_AURA_FRAME_ICON_DIRECTION_RIGHT)}
}
...
--备注
    root:CreateButton(
        '|A:dressingroom-button-appearancelist-up:0:0|a'..(WoWTools_Mixin.onlyChinese and '备注' or LABEL_NOTE),
    function()
        WoWTools_TextMixin:ShowText(
            Save().noteText,
            WoWTools_Mixin.onlyChinese and '宏' or MACRO,
...
--布局
    ...
    sub=root:CreateButton(
        WoWTools_Mixin.onlyChinese and '布局' or HUD_EDIT_MODE_LAYOUT:gsub(HEADER_COLON, ''),
    function()
        return MenuResponse.Open
    end)
...
--按钮增强
    sub=root:CreateCheckbox(
        WoWTools_Mixin.onlyChinese and '按钮增强' or 'Button Plus',
    function()
...
--打开，选项界面
    ...
    sub:CreateTitle(WoWTools_Mixin.onlyChinese and '全部删除' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, DELETE, ALL))
    sub:CreateDivider()

--删除，通用宏
    num, num2= GetNumMacros()
    text= (WoWTools_Mixin.onlyChinese and '通用宏' or GENERAL_MACROS)..(num==0 and ' |cff9e9e9e#' or ' #')..num
    sub:CreateButton(
...
local function Create_Button(name)
   ...
   function btn:set_tooltips()
        ...
        if InCombatLockdown() then
            GameTooltip:AddLine(WoWTools_Mixin.onlyChinese and '战斗中' or HUD_EDIT_MODE_SETTING_ACTION_BAR_VISIBLE_SETTING_IN_COMBAT)
        else
...
--备注 EditBox
    NoteEditBox=WoWTools_EditBoxMixin:CreateMultiLineFrame(MacroFrame, {
        ...
        isInstructions= WoWTools_Mixin.onlyChinese and '备注' or LABEL_NOTE
    })
...
--目标
    TargetButton= Create_Button(WoWTools_Mixin.onlyChinese and '目标' or TARGET)
    ...
    TargetButton.tip2=WoWTools_Mixin.onlyChinese and '光环名称' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, AURAS, NAME)
...
--攻击
    AttackButton= Create_Button(WoWTools_Mixin.onlyChinese and '攻击' or ATTACK)
...
local function Init()
    ...
    Button.Text:SetText(WoWTools_Mixin.onlyChinese and '战斗中' or HUD_EDIT_MODE_SETTING_ACTION_BAR_VISIBLE_SETTING_IN_COMBAT)
