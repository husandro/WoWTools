...
function WoWTools_TooltipMixin:Set_Currency(tooltip, currencyID)--货币
    ...
    tooltip:AddDoubleLine(
        (icon or '')..(col or '')..(e.onlyChinese and '货币' or TOKENS)..' '..currencyID,
    ...
    if num>0 then
        ...
        if numPlayer>1 then
            ...
            --[[tooltip:AddDoubleLine(
                (col or '|cnGREEN_FONT_COLOR:')..numPlayer..(icon or e.Icon.wow2)..(e.onlyChinese and '角色' or CHARACTER),
                (icon or e.Icon.wow2)..(col or '|cnGREEN_FONT_COLOR:')..WoWTools_Mixin:MK(num, 3)
            )]]
        end
    end
