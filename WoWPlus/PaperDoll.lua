...
local Icon={
    ...
    O='Interface\\AddOns\\WeakAuras\\Media\\Textures\\cancel-mark.tga',--￠
    pu='Forge-ColorSwatchHighlight',
    s='AlliedRace-UnlockingFrame-Checkmark',
    equi='FXAM-SmallSpikeyGlow',
...
local function Sever()--显示服务名称
    ...
    if not Save.hide then
        if not s then
            s=Cstr(Frame);
            s:SetPoint('TOPLEFT',60 ,-6);
            s:SetJustifyH('LEFT');
            
            s.tip=CreateFrame("Frame",nil, Frame);
            s.tip:SetPoint('TOPLEFT',60 ,-6);
            s.tip:SetSize(60, 18);
            s.tip:SetScript("OnEnter",function(self)
                    ...
                    for k, v in pairs(GetAutoCompleteRealms()) do
                        if v==Player.ser then
                            GameTooltip:AddDoubleLine(v, k, 0,1,0);
                        else
                    ...
                    if not ok2 then
                        GameTooltip:AddLine(ITEM_UNIQUE,    SERVER_MESSAGE_PREFIX);
                    end
                    GameTooltip:Show();
            end);
            s.tip:SetScript("OnLeave",function() GameTooltip:Hide() end);
            Frame.server=s;
        end
        ok=true;
        s:SetText(Player.ser);
        s.tip:SetSize(s:GetSize());
    end
    if s then s:SetShown(ok) end
end
Sever();
...
local function LvTo()--总装等
   ...
   --[[ if lv and lv>7 then
        Lib.AutoCastGlow_Start(f, nil,  2, 0.20, 1);
    else
        Lib.AutoCastGlow_Stop(f);
    end]]
end
...
local function Engineering(self, slot, use)--增加 [潘达利亚工程学: 地精滑翔器][诺森德工程学: 氮气推进器]    
    ...
    self.engineering:SetScript('OnEnter' ,function(self2)
            ...
            GameTooltip:ClearLines()
            GameTooltip:SetSpellByID(self2.spell);
            GameTooltip:Show();
    end); 
    ...
    self.engineering:SetScript('OnLeave',function() GameTooltip:Hide() end);        
end
local enchantStr=ENCHANTED_TOOLTIP_LINE:gsub('%%s','');--附魔
local function Enchant(self, slot, link)--附魔, 使用, 属性
    local enchant, use;
    if link and not Save.hide then
        local Tips = _G['ScannerTooltip'] or CreateFrame('GameTooltip', 'ScannerTooltip', e.region, 'GameTooltipTemplate');    
        Tips:SetOwner(UIParent, "ANCHOR_NONE");
        Tips:ClearLines();
        Tips:SetHyperlink(link);
        
        for i=1, Tips:NumLines() do
...
local function ADDEquipment(equipmentSetsDirty)--添加装备管理框
    ...
    if not f then
        ...
        f:SetScript("OnEnter", function (self)
                GameTooltip:SetOwner(self, "ANCHOR_LEFT");
                GameTooltip:ClearLines();
                GameTooltip:AddDoubleLine(addName, nil, 1,1,1);        
                GameTooltip:AddDoubleLine(ADD, EQUIPMENT_MANAGER);
                GameTooltip:AddDoubleLine(Save.EquipmentH and BINDING_NAME_STRAFERIGHT or BINDING_NAME_PITCHDOWN, Icon.left);
                GameTooltip:AddDoubleLine(NPE_MOVE, Icon.right);
                GameTooltip:AddDoubleLine(ZOOM_IN..'/'..ZOOM_OUT..': '..(Save.zoom and Save.zoom or 1), Icon.mid);
                GameTooltip:Show();
        end);        
...
local function GetDurationTotale(self)--装备总耐久度
    local cu, max=0,0;
    if not Save.hide then
...
Frame.sel:SetCheckedTexture(Icon.O);
Frame.sel.Text:SetJustifyH('LEFT');
...
Frame.sel:SetScript("OnEnter", function (self)
    GetDurationTotale(self)
        GameTooltip:SetOwner(self, "ANCHOR_BOTTOMLEFT");
        GameTooltip:ClearLines();
        GameTooltip:AddDoubleLine(addName, nil, 1,1,1);
        if self.DuVal and self.DuVal~='' then
            GameTooltip:AddDoubleLine(DURABILITY, self.DuVal..'%');
        end
        GameTooltip:AddDoubleLine(SHOW..'/'..HIDE, Save.hide and HIDE or SHOW, nil,nil,nil, 0,1,0);
        GameTooltip:Show();
end);
...
Frame.sel:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == id then
        Save= PaperDollSave or {}
        ADDEquipment()--装备管理框
        GetDurationTotale(self)--装备总耐久度
        
        Frame.sel:SetChecked(Save.hide);
        if Save.show then
            Frame.sel2:SetNormalAtlas(Icon.equi);
        else
            Frame.sel2:SetNormalTexture(Icon.O);
        end
...
Frame.sel2:SetScript("OnClick", function(self)
        if Save.show then 
            Save.show=nil;
            self:SetNormalTexture(Icon.O);
        else 
            Save.show=true;
            self:SetNormalAtlas(Icon.equi);
        end
        ADDEquipment();
end)
Frame.sel2:SetScript("OnEnter", function (self)
        ...
        GameTooltip:AddDoubleLine(addName, nil, 1,1,1);        
        GameTooltip:AddDoubleLine(ADD, EQUIPMENT_MANAGER);
        GameTooltip:AddDoubleLine(SHOW..'/'..HIDE, Save.show and SHOW or HIDE, nil,nil,nil, 0,1,0);
        GameTooltip:Show();
end);
