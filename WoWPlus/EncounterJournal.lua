...
local function set_EncounterJournal_World_Tips(self2)--所有角色已击杀世界BOSS提示
    ...
    for guid, info in pairs(WoWDate) do
        ...
        if find then
            e.tips:AddDoubleLine(e.GetPlayerInfo({unit=nil, guid=guid, name=nil, reFriendFaction=true, reName=false, reRealm=false, reLink=false}), guid==e.Player.guid and e.Icon.star2)
        end
    end
    e.tips:Show()
end
...
local function setInstanceBossText()--显示副本击杀数据
    ...
    if not Save.hideInstanceBossText then
        for guid, info in pairs(WoWDate) do
            ...
            if text then
                msg=msg and msg..'\n' or ''
                msg= msg ..e.GetPlayerInfo({unit=nil, guid=guid, name=nil, reFriendFaction=true, reName=true, reRealm=true, reLink=false})..'\n'
                msg= msg.. text
            end
        end
        msg=msg or '...'
...
local function set_EncounterJournal_Keystones_Tips(self)--险指南界面, 挑战
    ...
    for guid, info in pairs(WoWDate) do
        if guid and info then
            ...
            if find then
                e.tips:AddLine(e.GetPlayerInfo({unit=nil, guid=guid, name=nil, reFriendFaction=true, reName=true, reRealm=true, reLink=false}))
            end
        end
    end
    e.tips:Show()
...
local function set_EncounterJournal_Money_Tips(self)--险指南界面, 钱
    ...
    for guid, info in pairs(WoWDate) do
        if info.Money then
            e.tips:AddDoubleLine(e.GetPlayerInfo({unit=nil, guid=guid, name=nil, reFriendFaction=true, reName=true, reRealm=true, reLink=false}), GetCoinTextureString(info.Money))
