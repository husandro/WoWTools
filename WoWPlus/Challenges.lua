...
local function set_UI_Blizzard_ChallengesUI()--史诗钥石地下城, 界面
    ...
    local function makeAffix(parent, id)
        local frame = CreateFrame("Frame", nil, parent);
        frame:SetSize(h*2+2, h*2+2);
        ...
        frame:SetUp(id);--Blizzard_ScenarioObjectiveTracker.lua
        return frame;
    end
            ...
            for k,v in pairs(affixs) do
                for i=3 ,1, -1 do         
                    if not ChallengesFrame['AffixOne'..k..i] then 
                        ...
                        if not last then 
                            ChallengesFrame['AffixOne'..k..i]:SetPoint('RIGHT', -10, -((k-1)*(h*2+8)));
                        else
    ...
    local function GetNum(mapID, all)--取得完成次数,如 1/10
        ...
        if self.mapID and not self.encounter then--打开冒险指南
    ...
    --if ChallengesFrame_Update then
        --hooksecurefunc('ChallengesFrame_Update', set)
    --elseif ChallengesFrame.Update then
        hooksecurefunc(ChallengesFrame, 'Update', set)
    --elseif ChallengesFrameMixin then
        --hooksecurefunc(ChallengesFrameMixin, 'Update', set)
...
panel:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        ...
        elseif arg1=='Blizzard_ChallengesUI' then--挑战,钥石,插入界面
            ...
            set_UI_Blizzard_ChallengesUI()--史诗钥石地下城, 界面
        end
