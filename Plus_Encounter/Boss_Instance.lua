...
local function Create_WorldBoss_Button(frame)
    ...
    function btn:set_tooltip()
        ...
        GameTooltip:AddDoubleLine(e.onlyChinese and '冒险指南' or ADVENTURE_JOURNAL, e.onlyChinese and '副本' or INSTANCE)
        GameTooltip:AddLine(' ')
        GameTooltip:AddDoubleLine(e.GetShowHide(not Save().hideInstanceBossText), e.Icon.left)
        GameTooltip:AddDoubleLine(e.onlyChinese and '移动' or NPE_MOVE, e.Icon.right)
        GameTooltip:AddDoubleLine(e.onlyChinese and '字体大小' or FONT_SIZE, (Save().InsFontSize or 12)..e.Icon.mid)
        GameTooltip:Show()
    end
