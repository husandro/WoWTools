    ...
    for class= 1, GetNumClasses() do
        ...
        if classInfo and classInfo.classFile then
            ...
            for dungeonEncounterID, specID in pairs(Save().loot[classInfo.classFile]) do
                ...
                if type(specID)=='table' then
                    specID= specID.specID
                    journalEncounterID= specID.bossID
                end
                local bossName= journalEncounterID and EJ_GetEncounterInfo(journalEncounterID)
                ...
                sub2=sub:CreateCheckbox(
                    ...
                    ..bossName,
                function(data)
                    local d= Save().loot[data.class][data.dungeonEncounterID]
...
--BOSS 列表
    WoWTools_DataMixin:Hook(EncounterBossButtonMixin, 'Init', function(btn, data)--{data={bossID index link rootSectionID, desctiption, name} }
        if Save().hideEncounterJournal or not data or not data.bossID then
        ...
        if btn.encounterID  then
            name, _, journalEncounterID, _, _, _, dungeonEncounterID= EJ_GetEncounterInfo(data.bossID)--data.bossID
...
function WoWTools_EncounterMixin:Init_Specialization_Loot2()
    Init()
end
