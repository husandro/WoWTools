...
local function createCheckerList(spellList, itemList, interactList)
  ...
  if spellList then
    for i = 1, #spellList do
      local sid = spellList[i]
      local name, minRange, range, spellIdx
...
function lib:processItemRequests(itemRequests)
  while true do
    ...
    while true do
      ...
      elseif item and C_Item.GetItemInfo(item) then
      ...
      elseif item and not itemRequestTimeoutAt[item] then
        ...
        return true
      elseif item and GetTime() >= itemRequestTimeoutAt[item] then
...
function lib:startMeasurement(unit, resultTable)
  ...
  if spellList then
    for i = 1, #spellList do
      ...
      local spellIdx = name and findSpellIdx(name)
      if name and spellIdx then
  ...
  for i = 1, #spellList do
    ...
    if (not name) or (name == "") or not range then
      ...
      if not spellIdx then
        ...
        local res
        res = IsSpellBookItemInRange(spellIdx, BOOKTYPE_SPELL, "target")
        if res == nil or verbose then
          if res == nil then
            res = "|cffed0000nil|r"
          end
...
local GetPlayerMapPosition = function(unit)
    local map = C_Map.GetBestMapForUnit(unit)
    local pos = map and C_Map.GetPlayerMapPosition(map, unit)
    if pos then
      return pos:GetXY()
    end
  end
