...
local function UpdateMountDisplay()
    ...
    if not MountJournal.MountDisplay.tipButton then
        ...
        function MountJournal.MountDisplay.tipButton:set_Tooltips()
            ...
            GameTooltip:AddDoubleLine(WoWTools_Mixin.onlyChinese and '显示信息' or format(CLUB_FINDER_LOOKING_FOR_CLASS_SPEC, SHOW, INFO), e.GetShowHide(not Save().ShowMountDisplayInfo))
            GameTooltip:AddLine(' ')
